...
if [[ "${1:-}" = "--tmux" ]]; then
  ...
  tmux new-session -d -s "$SESSION" -x 200 -y 50 \
    "cd '$REPO_ROOT' && PI_CODING_AGENT_DIR='$PI_CODING_AGENT_DIR' PI_TELEGRAM_CONFIG='$PI_TELEGRAM_CONFIG' PI_MEMORY_DIR='$PI_MEMORY_DIR' QMD_CONFIG_DIR='$QMD_CONFIG_DIR' XDG_CACHE_HOME='$XDG_CACHE_HOME' PATH='$PATH' '$BIN_PATH' 2>'$REPO_ROOT/runtime/log/$SESSION-stderr.log' | tee '$REPO_ROOT/runtime/log/$SESSION-tmux.log'"
